...
function a(a2, s2 = "metric") {
  ...
  if ("best" === a2) {
    ...
    return a3 = r(a3, i2).to(o2), { quantity: a3, unit: o2, toString: this.k ? () => a3 + o2 : (e3) => (e3 === void 0 ? a3 : a3.toFixed(e3)) + o2 };
  }
  const m2 = e[a2];
  if (this.u[0] !== m2[0])
    throw new RangeError();
  if (this.k) {
    if (this.i in i || a2 in i)
      throw new RangeError();
    return this.o * BigInt(this.u[1] / m2[1]);
  }
  if (this.q) {
    switch (a2) {
      case "K":
    ...
    switch (this.i) {
      ...
      case "kelvin":
      case "kelvins":
        return a2 in i ? this.o / m2[1] - i[a2] : this.o / m2[1];
    }
    return r(r(this.o, this.i).to("K"), "K").to(a2);
  }
  return this.o * (this.u[1] / m2[1]);
}
function r(t2, i2) {
  const r2 = typeof t2, s2 = "bigint" === r2;
  if (!s2 && "number" !== r2)
    throw new TypeError();
  const m2 = e[i2];
  if (!m2)
    throw new RangeError();
  return { to: a.bind({ o: t2, i: i2, u: m2, k: s2, q: 9 === m2[0] }) };
}
var s = /(-?(?:\d+)?\.?\d+)(\S+)/g;
function m(e2, t2) {
  const i2 = "best" === e2;
  let a2, m2 = 0, o2 = true;
  do {
    const t3 = r(Number(this.P[1]), this.P[2]).to(i2 && !o2 ? a2 : e2);
    i2 && o2 ? (m2 += t3.quantity, a2 = t3.unit, o2 = false) : m2 += t3, this.P = s.exec(this.v);
  } while (this.P);
  return i2 ? r(m2, a2).to("best", t2) : m2;
}
function o(e2) {
  s.lastIndex = 0;
  const t2 = s.exec(e2);
  if (!t2)
    throw new RangeError();
  return { to: m.bind({ P: t2, v: e2 }) };
}
function c(e2) {
  return "string" == typeof e2 ? o(e2).to("ms") : "" + r(e2, "ms").to("best");
}
export {
  r as convert,
  o as convertMany,
